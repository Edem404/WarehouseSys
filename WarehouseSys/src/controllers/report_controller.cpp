...
std::string ReportController::generate_warehouse_state_report(json document_generation_params)
{
    if (!document_generation_params.contains("date_to"))
        throw std::runtime_error("Missing required field: date_to");
    if (!document_generation_params.contains("report_type"))
    ...
    return _report_service->generate_warehouse_state_report(report_type, report_format, report_param_query);
}
